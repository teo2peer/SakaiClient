...
import { buildResourceTree, resourceFolderView, resourceTreeSummary, visibleResourceRows, type ResourceTreeNode } from '@/lib/resource-tree';
...
export default function CourseScreen() {
  const { id, folder } = useLocalSearchParams<{ id: string; folder?: string | string[] }>();
  const app = useApp();
  ...
  const folderNavigationMode = app.data.settings.folderNavigationMode;
  const expanded = expandedFolders.courseId === id ? expandedFolders.paths : new Set<string>();
  const openFolderPath = Array.isArray(folder) ? folder[0] ?? '' : folder ?? '';
  const folderView = resourceFolderView(tree, openFolderPath);
  const rows = folderNavigationMode === 'screen' ? folderView.rows : visibleResourceRows(tree, expanded);
  const viewingFolder = folderNavigationMode === 'screen' && folderView.breadcrumbs.length > 0;
  ...
  function openOrToggleFolder(path: string) {
    if (folderNavigationMode === 'screen') {
      router.push({ pathname: '/course/[id]', params: { id, folder: path } });
    }
    else toggleFolder(path);
  }

  function navigateUp(levels: number) {
    router.dismiss(levels);
  }
  ...
  return (
    <AppShell>
      <FlatList
        key={folderNavigationMode === 'screen' ? openFolderPath : 'expandable'}
        data={rows}
        ...
        <View className="gap-4 px-5 pb-5 pt-5">
          {viewingFolder ? (
            <FolderBreadcrumb
              breadcrumbs={folderView.breadcrumbs}
        ...
        <View className="gap-3 px-5">
          ...
          {!viewingFolder ? <View className="flex-row items-center justify-between border-b border-zinc-200 py-3 dark:border-zinc-800">
          ...
          {folderNavigationMode === 'screen' && !viewingFolder ? (
            <FolderBreadcrumb
              breadcrumbs={folderView.breadcrumbs}
              onNavigate={navigateUp}
            />
            ...
            <View className="mx-5" style={{ paddingLeft: Math.min(depth, 5) * 14 }}>
              <View className="flex-row items-center border-b border-zinc-200 dark:border-zinc-800">
                <Pressable
                  ...
                  accessibilityState={node.isFolder
                    ? folderNavigationMode === 'expandable' ? { expanded: isExpanded } : undefined
                    : { disabled }}
                  accessibilityHint={`${node.isFolder
                    ? folderNavigationMode === 'screen' ? 'Pulsa para abrir su contenido' : 'Pulsa para mostrar u ocultar su contenido'
                    : document ? 'Abrir el visor' : 'Ver opciones de descarga y lectura'}. Usa el botón de acciones para más opciones`}
                  disabled={disabled}
                  onPress={() => {
                    if (node.isFolder) openOrToggleFolder(node.path);
                  ...
                  {node.isFolder ? (
                    <View className="h-10 w-10 items-center justify-center rounded-lg bg-mint">
                      {folderNavigationMode === 'screen' ? (
                        <Image
                          source={require('@/assets/icons/folder.svg')}
                          contentFit="contain"
      ...
      {actionRow ? (
        <ResourceActions
          ...
          onFolderPress={() => openOrToggleFolder(actionRow.node.path)}
...
function FolderBreadcrumb({
  ...
  breadcrumbs: ResourceTreeNode[];
  onNavigate(levels: number): void;
}) {
  const items = [{ path: '', name: 'Recursos' }, ...breadcrumbs];
  return (
    <ScrollView
      horizontal
      contentContainerClassName="items-center gap-1 py-1"
      showsHorizontalScrollIndicator={false}>
      {items.map((item, index) => {
        const current = index === items.length - 1;
        return <View key={item.path || 'root'} className="flex-row items-center gap-1">
          {index > 0 ? <Text className="text-xs text-zinc-400">›</Text> : null}
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={current ? `Carpeta actual: ${item.name}` : `Ir a ${item.name}`}
            accessibilityState={{ disabled: current }}
            disabled={current}
            onPress={() => onNavigate(items.length - index - 1)}
            className="min-h-11 max-w-[180px] justify-center rounded-md px-1 active:bg-mint">
            <Text
              numberOfLines={1}
              className={`text-xs ${current ? 'font-semibold text-ink dark:text-zinc-100' : 'font-medium text-pine'}`}>
              {item.name}
            </Text>
          </Pressable>
        </View>;
